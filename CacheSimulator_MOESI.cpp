...
enum MOESI {
    M,  // Modified: dirty, private (only this cache has it)
    O,  // Owned: dirty, shared, owner (responsible for writeback, must supply on BusRd)
    E,  // Exclusive: clean, private (only this cache has it, matches memory)
    S,  // Shared: clean (or stale if O exists), read-only
    I   // Invalid: not present or not valid
};
...
struct Monitor {
    ...
    int bus_invalidate_update_cnt = 0;  // For MOESI: counts invalidations

    int private_data_accesses = 0;  // Access to M or E state
    int shared_data_accesses = 0;   // Access to S or O state
    ...
    void print_statistics() const {
        std::cout << "\n========================================" << std::endl;
        std::cout << "        SIMULATION RESULTS (MOESI)" << std::endl;
        std::cout << "========================================\n" << std::endl;
        ...
        // 8. Invalidations on bus (MOESI uses invalidation protocol)
        std::cout << "8. Invalidations on Bus: "
...
// MOESI Protocol Bus Commands
enum Cmd {
    BusRd,    // Read block (shared)
    BusRdX,   // Read block for ownership (exclusive/write)
    BusUpgr,  // Upgrade S/O to M (invalidate others, no data transfer)
...
struct BusTxn {
    ...
    int supplied_by_cache = 0; // 0: none, 1: E/S supplies (clean), 2: M/O supplies (dirty)
...
private:
    ...
    public:
        ...
        Entry() {}
        Entry(int address, std::vector<int> words, MOESI state): address(address), words(words), state(state) {}
    };
    class Request {
    public:
        Cmd cmd;
...
public:
    ...
    bool evict(int index) {
        ...
        return (state == MOESI::M || state == MOESI::O);
    }
    void print() {
...
bool LRU_Cache::snoop(Cmd cmd, int address, std::optional<BusTxn>& active) {
    ...
    if (cmd == Cmd::BusRd) {
        ...
        // MOESI BusRdX snooping - invalidate all sharers
        if (entry.state == MOESI::M) {
            // M → I: Supply dirty data, invalidate
            (*active).supplier_core = core->get_id();
            (*active).supplied_by_cache = 2;  // Dirty supply
            entry.state = MOESI::I;
            invalidated = true;
        } else if (entry.state == MOESI::O) {
            // O → I: Supply dirty data, invalidate (owner loses ownership)
            (*active).supplier_core = core->get_id();
            (*active).supplied_by_cache = 2;  // Dirty supply
            entry.state = MOESI::I;
            invalidated = true;
        ...
        // MOESI BusUpgr snooping - invalidate S and O sharers
        if (entry.state == MOESI::S) {
            // S → I: Invalidate
            entry.state = MOESI::I;
            invalidated = true;
        } else if (entry.state == MOESI::O) {
            // O → I: Invalidate (lose ownership)
            entry.state = MOESI::I;
            invalidated = true;
        }
        // E and M should not see BusUpgr (requester would be I, not S/O)
        ...
        if (pending.size()) {
            ...
            if (cmd == Cmd::BusRd) {
                ...
                if ((*active).supplied_by_cache == 2) {
                    ...
                    for (auto& core: cores) {
                        if (core->get_id() == (*active).src_core) {
                            core->get_cache()->update_entry_state((*active).address, MOESI::S);
                            break;
                        }
                    }
                    ...
                    for (auto& core: cores) {
                        if (core->get_id() == (*active).src_core) {
                            core->get_cache()->update_entry_state((*active).address, MOESI::E);
                            break;
                        }
                    }
                }
                ...
                for (auto& core: cores) {
                    if (core->get_id() == (*active).src_core) {
                        core->get_cache()->update_entry_state((*active).address, MOESI::M);
                        break;
                    }
                }
            } else if (cmd == Cmd::BusUpgr) {
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "  Protocol: MOESI" << std::endl;
